...
pub const Term = struct {
    ...
    /// Single bytes only: the dashboard's keys are all unmodified letters. The poll is
    /// what distinguishes "nothing yet" from "nothing ever again" — a VTIME read returns
    /// zero for both.
    pub fn readKey(t: *Term) Key {
        var fds = [_]posix.pollfd{.{ .fd = t.tty, .events = posix.POLL.IN, .revents = 0 }};
        const ready = posix.poll(&fds, 100) catch return .timeout;
...
pub fn paint(fd: posix.fd_t, current: Screen, previous: ?Screen, out: *std.ArrayList(u8), gpa: std.mem.Allocator) !void {
    ...
    for (0..current.h) |y| {
        ...
        for (current.row(y)) |cell| {
            if (cell.style.fg != style.fg or cell.style.bold != style.bold) {
                style = cell.style;
                try out.print(gpa, "\x1b[0;{s}m", .{style.fg.sgr()});
...
test "colour is emitted when it changes and reset at the end of a row" {
    ...
    defer out.deinit(testing.allocator);
    try paint(-1, s, null, &out, testing.allocator);
    try testing.expect(std.mem.indexOf(u8, out.items, "\x1b[0;32m") != null);
